...
Examples:
    ...
    async def tempban(
        ...
        *,
        ...
        dm_message = f"""**NOTICE: Temporary Ban from {guild_name} Discord Server**

Dear {user.display_name},

We are writing to inform you that you have been temporarily banned from the {guild_name} Discord server for **{duration}**.
...
🔗 [Appeal Form](https://example.com/appeal-form)
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    ...
                    description=(
                        f"I could not send a warning to {user.mention} because they block server DMs. "
                        "Please contact them in another channel if needed."
                    ),
                    color=self.error_colour,
                ),
                ephemeral=True,
...
**Moderators Involved:** {moderator.mention}"""
                    ...
                    await interaction.followup.send(
                        embed=discord.Embed(
                            title="Log Failed",
                            description=(
                                f"I could not post the temporary ban in [logs channel]({log_link}). "
            ...
            await interaction.followup.send(
                embed=discord.Embed(
                    ...
                    description=(
                        "Set `channels.moderation_logs_channel_id` in config.yaml so I know where to post temporary ban logs."
                    ),
                    color=self.error_colour,
                ),
                ephemeral=True,
