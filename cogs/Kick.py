...
class Kick(commands.Cog):
    ...
    async def kick(
        ...
        try:
            try:
                dm_text = f"""**NOTICE: Kick from {guild_name} Discord Server**

Dear {user.mention},

We are writing to inform you that you have been kicked from the {guild_name} Discord server. This action is a result of your violations of our community rules.
...
"""
                ...
                if e.status == 403:  # DMs Disabled
                    ...
                    embed = discord.Embed(
                        ...
                        description=(
                            "I could not send the notice because that member blocks server DMs. "
                            "Please contact them in-channel or ask them to enable DMs."
                        ),
                    ...
                    embed = discord.Embed(
                        ...
                        description=(
                            f"I could not send the notice to {user.mention}. "
                            "Please verify the user can receive DMs or notify them another way."
                        ),
                        color=self.error_colour,
                    )
                    await interaction.followup.send(embed=embed)
            try:
                ...
                if e.status == 403:  # Bot has no permission to kick
                    ...
                    embed = discord.Embed(
                        ...
                        description=(
                            "I cannot kick members with my current permissions or role position. "
                            "Please give me the Kick Members permission and place my role above the member you are removing."
                        ),
                        color=self.error_colour,
                    )
                    await interaction.followup.send(embed=embed)
                    ...
                    embed = discord.Embed(
                        title="Kick Failed",
                        description=(
                            f"I could not remove {user.mention}. Please try again in a moment or check the audit log for details."
                        ),
                        color=self.error_colour,
                    )
                    await interaction.followup.send(embed=embed)
                ...
                if not logs_channel_id:
                    embed = discord.Embed(
                        ...
                        description=(
                            "Set `channels.moderation_logs_channel_id` in config.yaml so I know where to send moderation logs."
                        ),
                        color=self.error_colour,
                    )
...
**Moderators Involved:** {moderator.mention}"""
                        ...
                        if e.status == 403:
                            ...
                            embed = discord.Embed(
                                ...
                                description=(
                                    f"I cannot write in {log_link}. Give me Send Messages permission in that channel or update the log channel in config.yaml."
                                ),
                                color=self.error_colour,
                            )
                            await interaction.followup.send(embed=embed)
                            ...
                            embed = discord.Embed(
                                ...
                                description=(
                                    "The configured log channel no longer exists. Update the channel ID in config.yaml or create a replacement channel."
                                ),
                                color=self.error_colour,
                            )
                            await interaction.followup.send(embed=embed)
                            ...
                            embed = discord.Embed(
                                title="Log Failed",
                                description=(
                                    f"Discord did not accept my log message for {log_link}. Please try again later."
                                ),
                                color=self.error_colour,
                            )
                            await interaction.followup.send(embed=embed)
                            ...
                            embed = discord.Embed(
                                title="Log Failed",
                                description=(
                                    f"I could not post in {log_link}. Check channel permissions or try again later."
                                ),
                                color=self.error_colour,
                            )
                            await interaction.followup.send(embed=embed)
            ...
            embed = discord.Embed(
                title="Kick Failed",
                description=(
                    f"I ran into an unexpected issue while kicking {user.mention}. Please review my permissions and try again."
                ),
                color=self.error_colour,
            )
            await interaction.followup.send(embed=embed)
