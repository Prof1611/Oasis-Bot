...
class Ban(commands.Cog):
    def __init__(self, bot):
        ...
        self.moderation_config = self.config["features"]["moderation"]
    ...
    async def ban(
        ...
        try:
            try:
                dm_text = f"""**NOTICE: Permanent Ban from the Oasis Discord Server**

Dear {user.mention},

We are writing to inform you that you have been permanently banned from the Oasis Discord server. This action is a result of your repeated violations of our community rules, despite previous warnings and attempts to rectify the situation.
...
**Appeal Process:** If you wish to appeal this ban, you may do so by filling out the following form: {self.moderation_config["ban_appeal_form_url"]}

Sincerely,
The Oasis Moderation Team
*Please do not reply to this message as the staff team will not see it.*"""
                ...
                if e.status == 403:  # DMs Disabled
                    ...
                    embed = discord.Embed(
                        title="Error",
                        description="That user has their DMs disabled. Failed to send notice.",
            ...
            logs_channel_id = self.moderation_config["logs_channel_id"]
            guild = interaction.guild
